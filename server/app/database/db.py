...
from app.database.models import Base, User, City, Category, Cuisine, Activity, ActivityAvailability, FoodOption, FoodOptionAvailability
...
def generate_itinerary():
    with Session(engine) as session:
        statement = select(User).filter_by(email="ed")
...
def fill_dummy_data():
    ...
    for _ in range(15):  # For each activity
        for day in range(7):  # For each day of the week
            if activity_id in [6, 11, 15]:  # Special hours for some activities
                open_time = time(8, 0)
                close_time = time(17, 0)
            elif activity_id in [9, 14]:  # Mall and entertainment zone
                open_time = time(10, 0)
    ...
    for _ in range(15):  # For each food option
        for day in range(7):  # For each day of the week
            if food_option_id in [1, 2, 7, 12, 14, 15]:  # Local eateries and sweet shops
                open_time = time(7, 0)
                close_time = time(22, 0)
                has_breakfast = True
                has_lunch = True
                has_dinner = True
            elif food_option_id in [3, 5, 8]:  # Fine dining and rooftop restaurants
                open_time = time(12, 0)
                close_time = time(23, 0)
                has_breakfast = False
                has_lunch = True
                has_dinner = True
            elif food_option_id in [4, 10]:  # Street food and late-night eateries
                open_time = time(16, 0)
                close_time = time(1, 0)  # Opens at 4 PM, closes at 1 AM next day
                has_breakfast = False
                has_lunch = False
                has_dinner = True
            else:  # Regular restaurants
                open_time = time(11, 0)
